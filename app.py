...
# loading the models
    ...
    file_path = r"C:\Users\Goutham Krishna\Desktop\multiple disease predictor\Multiple-Disease-Prediction-Webapp-main\disease.csv"
    df = pd.read_csv(file_path)
    df.columns = ["Disease", "Doctor", "Details"]

    # Load Model  # Assuming your model is stored in 'model.py'
    disease_model = DiseaseModel()
    disease_model.load_xgboost('model/xgboost_model.json')

    # UI
    if selected == 'Disease Prediction': 
        image = Image.open('disea.jpg')
        ...
        if st.button('Predict'): 
            ...
            description = disease_model.describe_predicted_disease()
            precautions = disease_model.predicted_disease_precautions()
            while len(precautions) < 4:
                precautions.append("N/A")

            # Fetch doctor details dynamically
            df_filtered = df[df["Disease"].str.lower() == prediction.lower()]
            doctor, details = ("N/A", "No details available.") if df_filtered.empty else (df_filtered.iloc[0]["Doctor"], df_filtered.iloc[0]["Details"])

            # Create a combined DataFrame
            df_combined = pd.DataFrame({
                "Disease": [prediction],
                "Description": [description],
                "Precaution 1": [precautions[0]],
                "Precaution 2": [precautions[1]],
                "Precaution 3": [precautions[2]],
                "Precaution 4": [precautions[3]],
                "Doctor": [doctor],
                "Doctor Details": [details]
            })

            # Show data
            tab1, tab2,tab3= st.tabs(["Description", "Precautions","Doctor"])

            with tab1:
                st.write(disease_model.describe_predicted_disease())
                print(disease_model.describe_predicted_disease())
            ...
            with tab3:
                ...
                df.columns = ["Disease", "Doctor", "Phone no"]

                # Function to check disease detail
                disease_name = prediction.strip()
                    
                    # Search for the disease in the DataFrame
                result = df[df["Disease"].str.lower() == disease_name.lower()]
                    
                if not result.empty:
                    disease = result.iloc[0]["Disease"]
                    doctor = result.iloc[0]["Doctor"]
                    details = result.iloc[0]["Phone no"]
                        
                    # Print output in console
                    print(f"Disease: {disease}, Doctor: {doctor}, Details: {details}")            
                    # Display in Streamlit
                    st.write(f"**Disease:** {disease}")
                    st.write(f"**Doctor:** {doctor}")
                    st.write(f"**Phone no:** {details}")

            # Download CSV
            csv = df_combined.to_csv(index=False).encode("utf-8")
            st.download_button("Download CSV", csv, "disease_info.csv", "text/csv")

            # Generate PDF
            from fpdf import FPDF

            def generate_pdf(disease, description, precautions, doctor, details):
                pdf = FPDF()
                pdf.set_auto_page_break(auto=True, margin=15)
                pdf.add_page()
                pdf.set_font("Arial", size=12)
                pdf.cell(200, 10, "Disease Information", ln=True, align="C")
                pdf.ln(10)

                # Ensure all variables are strings to avoid AttributeError
                disease = str(disease)
                description = str(description)
                doctor = str(doctor)
                details = str(details)  # Convert details to string
                precautions = [str(prec) for prec in precautions]  # Convert list elements to string

                # Disease
                pdf.set_font("Arial", style="B", size=10)
                pdf.cell(20, 10, "Disease: ", ln=False)
                pdf.set_font("Arial", size=10)
                pdf.cell(0, 10, disease, ln=True)

                # Description
                pdf.set_font("Arial", style="B", size=10)
                pdf.cell(50, 10, "Description: ", ln=True)
                pdf.set_font("Arial", size=10)
                pdf.multi_cell(0, 10, description)

                # Precautions
                pdf.set_font("Arial", style="B", size=10)
                pdf.cell(50, 10, "Precautions: ", ln=True)
                pdf.set_font("Arial", size=10)
                for i, prec in enumerate(precautions):
                    pdf.cell(0, 10, f"{i+1}. {prec}", ln=True)

                # Doctor
                pdf.set_font("Arial", style="B", size=10)
                pdf.cell(20, 10, "Doctor: ", ln=False)
                pdf.set_font("Arial", size=10)
                pdf.cell(0, 10, doctor, ln=True)

                # Doctor Details
                pdf.set_font("Arial", style="B", size=10)
                pdf.cell(20, 7, "", ln=False)
                pdf.set_font("Arial", size=10)
                pdf.multi_cell(0, 0, details)

                # Save PDF
                pdf_path = "disease_info.pdf"
                pdf.output(pdf_path)
                return pdf_path

            pdf_path = generate_pdf(prediction, description, precautions, doctor, details)

            # Download PDF
            with open(pdf_path, "rb") as file:
                st.download_button("Download PDF", file, "disease_info.pdf", "application/pdf")
    ...
    # change the variables according to the dataset used in the model
        ...
        col1, col2, col3 = st.columns(3)
        with col1:
            MDVP = st.number_input("MDVP_FO(Hz)")
        with col2:
            MDVPFIZ = st.number_input("MDVP_Fhi(Hz)")
        with col3:
            MDVPFLO = st.number_input("MDVP_Flo(Hz)")
        with col1:
            MDVPJITTER = st.number_input("MDVP_Jitter(%)")
        with col2:
            MDVPJitterAbs = st.number_input("MDVP_Jitter(Abs)")
        with col3:
            MDVPRAP = st.number_input("MDVP_RAP")

        with col2:

            MDVPPPQ = st.number_input("MDVP_PPQ ")
        with col3:
            JitterDDP = st.number_input("Jitter_DDP")
        with col1:
            MDVPShimmer = st.number_input("MDVP_Shimmer")
        with col2:
            MDVPShimmer_dB = st.number_input("MDVP_Shimmer(dB)")
        with col3:
            Shimmer_APQ3 = st.number_input("Shimmer_APQ3")
        with col1:
            ShimmerAPQ5 = st.number_input("Shimmer_APQ5")
        with col2:
            MDVP_APQ = st.number_input("MDVP_APQ")
        with col3:
            ShimmerDDA = st.number_input("Shimmer_DDA")
        with col1:
    ...
    if selected == 'Liver prediction':  # pagetitle
        ...
        image = Image.open('human-liver-highlighted.jpg')
...
def main():
    ...
    if st.session_state.get("logged_in", False):
        ...
        st.warning("Please log in to access the models.")
